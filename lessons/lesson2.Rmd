...
- *Hint 2*: [You may specify either the columns to pivot, or the columns not to pivot. There is only one column not to pivot, `religion`.]{.spoiler}

# Answers

## Exercise 1

[`barnyard_ghg <- read.csv('datasets/barnyard_ghg.csv')`]{.spoiler}

## Exercise 2

[One way would be `glimpse(barnyard_ghg)` and `table(barnyard_ghg$Surface)`]{.spoiler}
...
[`pivot_longer(relig_income, cols = -religion)`, or if you want to give your own names to the newly created columns, `pivot_longer(relig_income, cols = -religion, names_to = 'income', values_to = 'individuals')`]{.spoiler}
