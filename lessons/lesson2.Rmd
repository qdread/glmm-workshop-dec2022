...
We can `filter()` by more than one condition, and `group_by()` more than one column. For example we could get the mean and standard deviation of all non-missing values of body weight for each combination of sex and life stage, for one particular species. (`PELE` = *Peromyscus leucopus*, the white-footed mouse).
...
> This is equivalent to `summarize(group_by(select(filter(dat))))` but in my opinion much more readable.
