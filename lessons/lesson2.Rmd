...
2. Examine the data frame. What are the data types of the different columns? How many different values are there in the `Surface` column?
  + *Hint*: [Use `str()`, `summary()`, and/or `glimpse()` to examine the data frame. Use `table()` or `unique()` for the `Surface` column.]{.spoiler}
...
6. Sort the result in increasing order of the mean value of `CO2_mgC`.
7. Load the `relig_income` example dataset which has survey data on the number of individuals of different religions by reported yearly income. (Use `data(relig_income)` to load the dataset.) Convert this dataset into "tidy" form so that each row has a religion, income level, and number of individuals.
  + *Hint 1*: [Use `pivot_longer()`.]{.spoiler}
...
2. [One way would be `glimpse(barnyard_ghg)` and `table(barnyard_ghg$Surface)`]{.spoiler}
3-6. [`barnyard_ghg %>%`  
`filter(Temp > 10) %>%`  
`group_by(Surface) %>%`  
`summarize(mean_CO2 = mean(CO2_mgC), mean_CH4 = mean(CH4_mgC), mean_NH3 = mean(NH3_ugN), mean_N2O = mean(N2O_ugN)) %>%`
`arrange(mean_CO2)`]{.spoiler}
7. [`pivot_longer(relig_income, cols = -religion)`, or if you want to give your own names to the newly created columns, `pivot_longer(relig_income, cols = -religion, names_to = 'income', values_to = 'individuals')`]{.spoiler}