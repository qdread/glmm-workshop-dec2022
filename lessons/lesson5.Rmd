...
The first example will use the Jansen apple dataset from the **agridat** package. Load it with `data('jansen.apple', package = 'agridat')` or if you are running R locally and don't have access to the **agridat** package, use `jansen.apple <- read_csv('datasets/jansen.apple.csv')`.
...
What is the average number of trees that got apple canker by inoculum level? By genotype? By combination of inoculum level and genotype?
  + *Hint*: [Use `group_by()`, `summarize()`, and `mean()`.]{.spoiler}
...
> NOTE: This dataset has a different format. Instead of one row per individual, the numbers of infected trees in each block are in each row. `glmer()` can take data in this format if you use `cbind(y, n - y)` as the response variable.

  + *Hint*: [Don't forget `family = binomial(link = 'logit')`!]{.spoiler}
...
<p class="spoiler">
```{r, eval = FALSE}
fit <- glmer(cbind(y, n - y) ~ gen + inoculum + gen:inoculum + (1|block), data = jansen.apple, family = binomial)
check_model(fit)
summary(fit)
```
...
For the next two exercises we will use another **agridat** dataset, the Beall webworms dataset from 1940. This dataset has counts of webworms in a beet field with two different insecticide treatments
