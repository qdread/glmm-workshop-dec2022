...
```{r}
lm_disease <- lm(disease ~ inoc, data = disease)
```
...
```{r}
check_model(lm_disease)
```


Modeling change in a binary response in this way can be done with a generalized linear model. We use a link function to convert the predicted response value into a scale that can be normally distributed. What we are predicting is the probability that an individual from each pen with or without the inoculation treatment will get the disease. The model formula is:

$$\text{logit}~\hat{y} = \beta_0 + \beta_1 inoc +$$
