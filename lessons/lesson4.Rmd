...
For these exercises, we will use the `besag.beans` dataset which is part of the **agridat** package, an R package that has a bunch of classic datasets from agricultural science. You can load the dataset using 

```{r, eval = FALSE}
data(besag.beans, package = 'agridat')
```

> NOTE: If you are running R locally and were unable to install the **agridat** package, I have provided the dataset as a CSV file: `besag.beans <- read_csv('datasets/besag.beans.csv')`

## Exercise 1

Examine the dataset. We have genotypes being grown in
