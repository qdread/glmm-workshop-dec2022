...
Examine the dataset. We have five genotypes of barley (`gen`) being grown across six locations in Minnesota (`loc`) in two different years (`year`): 1931 and 1932.
...
A) Fit a linear mixed model with `protein` as a response variable, genotype `gen` as a fixed effect, and combination of location `loc` and year `year` as a 
...
Fit a linear mixed model with biomass as the response variable, 
