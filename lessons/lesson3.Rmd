...
> NOTE: That isn't surprising here because I purposefully simulated data with almost zero variation in the treatment effect between fields. But this is something you see a lot in real datasets.
...
1. What is the `lmer()` model formula for a random intercepts mixed model where the fixed effects are `temperature`, `rainfall`, and their interactions, the grouping variable is `block`, and the response variable is `yield`?
2. 

# Answers

1. [`yield ~ temperature + rainfall + temperature:rainfall + (1 | block)`]{.spoiler}
2. 